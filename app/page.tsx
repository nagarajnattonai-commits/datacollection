...
export default function Home() {
  const [data, setData] = useState<Snapshot | null>(null),
    ...
    [role, setRole] = useState<Role>('admin'),
    [tab, setTab] = useState('contribute');
    ...
    <main className="workspace">
      <header className="topbar">
        ...
        <div className="actions">
          {data?.demo ? (
            <Choice
              label="Local demo role"
              value={role}
              onChange={(v) => {
                setRole(v as Role);
                setTab(v === 'qa' ? 'quick' : 'contribute');
                setError('');
              }}
              options={[
            ...
            <span className="pill">{actor?.role ?? 'Private workspace'}</span>
          )}
          <Button
            variant="ghost"
        ...
        <>
          {data.demo && (
            <div className="demo-note">
              Local pilot · Use the role selector to try each workflow.
              Recordings and reviews are saved on this computer.
            </div>
          )}
